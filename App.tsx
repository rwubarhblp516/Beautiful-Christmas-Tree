...
import { TreeColors, HandGesture, SharedTreeData } from './types';
import { loadCachedImagesWithKeys, saveImagesToCacheWithKeys, deleteCachedImages } from './utils/idb';
import { resolveAssetPath } from './utils/assetPath';
import { compressImageForShare, urlToBlob } from './utils/compression';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initialPreview = isPreview;

    // POST MESSAGE LISTENER (Robust state sync with parent iframe)
    const handleMessage = (event: MessageEvent) => {
      // Security check: ensure trusted origin if needed, but for local tools skip for now
      if (event.data && event.data.type === 'fullscreen-change') {
        if (event.data.fullscreen) {
          setIsPreview(false);
        } else {
          // Respect initial state
          setIsPreview(initialPreview);
        }
      }
    };
    ...
    const handleFullscreenChange = () => {
      // Fallback native check
      if (document.fullscreenElement) {
        setIsPreview(false);
      } else {
        // If we exit fullscreen, ONLY revert to preview if we started in preview mode.
        // Otherwise, stay in interactive mode.
        if (initialPreview) {
          setIsPreview(true);
        } else {
          setIsPreview(false);
    ...
    // Initial cleanup of body background just in case
    if (bg) {
  ...
  useEffect(() => {
    // If read-only/shared, force the starry night black background
    const targetColor = isReadOnly ? '#000000' : (bgColor ? bgColor : (isPreview ? 'transparent' : '#010a05'));
    document.documentElement.style.backgroundColor = targetColor;
    document.body.style.backgroundColor = targetColor;

    // Also force clean up if unmounting
    return () => {
      document.documentElement.style.backgroundColor = '';
      document.body.style.backgroundColor = '';
    };
  }, [isPreview, bgColor, isReadOnly]);
  ...
  const initAudio = useCallback(() => {
    if (!bgmRef.current) {
      const bg = new Audio(resolveAssetPath('audio/bgm.mp3'));
      bg.loop = true;
      bg.autoplay = true;
      bg.preload = 'auto';
  ...
  // Load cached images on first render (stored in IndexedDB) IF NOT SHARE MODE
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const sid = params.get('share_id');

    if (sid) {
      // Shared Mode: Load data from server
      setSharedId(sid);
      setIsReadOnly(true);
      setBgmReady(true); // Auto-assume ready, user will interact or it auto-plays

      fetch(`/api/share/${sid}`)
        .then(res => {
          if (!res.ok) throw new Error('Failed to load shared tree');
          return res.json<SharedTreeData>();
        })
        .then(data => {
          // Restore state
          // 1. Restore Images (convert base64 to blob urls)
          const records = data.images.map(img => {
            ...
            return { key: img.key, url: URL.createObjectURL(blob) };
          });
          ...
          setPhotoSignatures(data.signatures || {});

          // 4. Restore Transforms
          setPhotoTransforms(data.transforms || {});

          // 5. Set Formed State
          setTargetMix(1);
        })
        .catch(err => {
          console.error('Load shared tree error:', err);
          alert('无法加载分享的圣诞树，可能链接已失效');
          setIsReadOnly(false); // Fallback
        });
    } else {
  ...
  const handleShareTree = async (): Promise<string | null> => {
    ensureAudioStarted();
    try {
      // 1. Gather all data
      const payload: SharedTreeData = {
        images: [],
        cards: customCards,
        signatures: photoSignatures,
        transforms: photoTransforms,
      };

      // 2. Compress images
      // We need to fetch the blob from the blobURL, compress it, then base64 it
      const imagePromises = userImageRecords.map(async (record) => {
        try {
          const blob = await urlToBlob(record.url);
          const compressedBase64 = await compressImageForShare(blob);
          return { key: record.key, data: compressedBase64 };
        } catch (err) {
          console.warn('Failed to compress image:', record.key, err);
          return null;
        }
      });

      const images = (await Promise.all(imagePromises)).filter((i): i is { key: string; data: string } => i !== null);
      payload.images = images;

      // 3. Upload to Cloudflare Functions
      ...
      // Use hash routing for consistent behavior with page-loader
      return `${origin}/#christmas-tree?share_id=${json.id}`;
    } catch (err) {
  ...
  const handleFocusMedia = useCallback((entry: { kind: 'image' | 'card'; url?: string; message?: string; signature?: string | null; id?: string; cacheKey?: string; editable?: boolean }, screenPos?: { x: number; y: number }) => {
    ...
    // FORCE NON-EDITABLE IF READ_ONLY
    if (isReadOnly) {
      normalizedEntry.editable = false;
    }
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden`} style={{ backgroundColor: isReadOnly ? '#000000' : (bgColor ? bgColor : (isPreview ? 'transparent' : '#010a05')) }}>
      ...
      <audio ref={bgmRef} loop preload="auto">
        <source src={resolveAssetPath('audio/bgm.mp3')} type="audio/mpeg" />
      </audio>
      ...
      {/* LOADING OVERLAY - Only show if NOT in preview mode or if processing uploads */}
      {(isProcessing || (!bgmReady && !isPreview)) && !isPreview && (
      ...
      {/* CENTER TITLE - Ethereal Silver Script - Hidden in Preview */}
      ...
      {/* Only show these if NOT read-only */}
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-6 z-50">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19.5 12c.5-2.5-3-5.5-7.5-5.5S4.5 9.5 5 12m14.5 0c-.5 2.5-3 5.5-7.5 5.5S4.5 14.5 5 12m14.5 0H5m7.5-5.5v11" />
            </svg>
          </button>
          ...
          <button
            onClick={handleUploadClick}
            className={`${iconButtonClass} border-white/40 bg-white/10 scale-110 hover:scale-125`}
            title="上传照片"
          >
      ...
      {isReadOnly && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2">
          <div className="px-4 py-2 rounded-full bg-black/40 backdrop-blur-md border border-white/10 text-white/70 text-sm font-luxury uppercase tracking-widest">
            Only for you
          </div>
        </div>
      )}
      ...
      {!isPreview && (
        <div className="absolute top-6 right-6 flex flex-col gap-4 z-50">
          {/* SHARE BUTTON - Visible if NOT read-only */}
          {!isReadOnly && (
            <button
